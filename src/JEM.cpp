                     ...
                     std::vector<std::unordered_map<kmer_t, std::vector<int>> > Tl, int start, int total_subjects, int total_q)
{
    ...
    int total_queries = 0;
    const std::string s0("/home/trahman/JEM_W_Sync/Strobemer/Previous_Max/Output/Mp1_all_mn_out_");
    char proc_id[3];
    ...
    while(p<length) {
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
        ...
        if(set_of_distinct_pos_rev.size() > 0)
        {
        for (int yu = 0; yu < set_of_distinct_pos_rev.size() - 1; yu++)
        {
            kmer_t kmer1 = set_of_distinct_kmers_rev[yu];
            int kmer_1_pos = set_of_distinct_pos_rev[yu];
            int range_start = kmer_1_pos + 1;
            int range_end = kmer_1_pos + 30;
            kmer_t min_strobe = 3074457345618258602;
            int next = yu+1;
            int min_tracker = 0;
            int cycle = 1;
            //std::cout<<rank <<" "<< kmer_1_pos<<" "<<  range_start<< " "<< range_end << "\n";
            while (next < set_of_distinct_pos_rev.size() && cycle == 1)
            
            {
                 if (set_of_distinct_pos_rev[next] >= range_start && set_of_distinct_pos_rev[next] < range_end)
                 {
                    if(min_strobe > set_of_distinct_kmers_rev[next] )
                    {
                        min_strobe = set_of_distinct_kmers_rev[next];
                        min_tracker = 1;
                    }
                    
                 }
                 if (set_of_distinct_pos_rev[next] >= range_end)
                 {
                     cycle = 0;
                 }
                 next += 1;
            }
            //std::cout<<rank <<" Tracker "<< kmer_1_pos<<" "<<  range_start<< " "<< range_end << "\n";
            if(min_tracker == 1)
            {
               set_of_distinct_kmers.push_back(kmer_merge(kmer1, min_strobe));
               set_of_distinct_pos.push_back(kmer_1_pos);   
            }
        }
        }
        std::cout<<rank <<" Tracker "<<total_queries <<" "<<  set_of_distinct_kmers_rev.size()<< " "<< set_of_distinct_kmers.size() << "\n";
     ...
     /*   if (rank == 0)
        ...
        while (j < set_of_distinct_kmers.size() && k < set_of_distinct_kmers.size())
        
        {
            if ((set_of_distinct_pos[k] - set_of_distinct_pos[j]) <= 950)
            {
                for(int g = 0; g < node_threashold; g++)
                {
           
                    kmer_t val = (Ax[g]*set_of_distinct_kmers[k] + Bx[g]) % Px[g];
            
                    if (val <minL[g])
                    {
                //minL[k] = ((Ax[k] * kmer_sets[j][i] + Bx[k]) % Px[k]);
                        minL[g] = val;
                        corr_kmer[g] = set_of_distinct_kmers[k];
                    }
                }  
                k += 1;
    ...
    while(p<length) {
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
       ...
       if(contig_len >= 1000 && kmer_set.size() > 0)
        ...
        set_of_distinct_pos.clear();
        set_of_distinct_pos.shrink_to_fit();
        //std::cout<< rank<<"RS"<<total_subjects<<"Dist_Kmer_Size Rev "<<set_of_distinct_kmers_rev.size()<<" "<< set_of_distinct_pos_rev.size()<<"\n";
        if(set_of_distinct_pos_rev.size() > 0)
        {
        for (int yu = 0; yu < set_of_distinct_pos_rev.size() - 1; yu++)
        {
            ...
            /*if (rank == 0)
            {
            std::cout<<rank <<" "<< kmer_1_pos<<" "<<  range_start<< " "<< range_end << " "<< min_strobe<<" "<< set_of_distinct_kmers_rev[yu]<<"\n";
            }*/
            while (next < set_of_distinct_pos_rev.size() && cycle == 1)
            
            {
                 /*if (rank == 0)
                 {
                     std::cout<<rank <<" Tracker "<<next <<" "<< kmer_1_pos<<" "<<  range_start<< " "<< range_end << "\n";
                 }*/
                 if (set_of_distinct_pos_rev[next] >= range_start && set_of_distinct_pos_rev[next] < range_end)
                 {
                    if(min_strobe > set_of_distinct_kmers_rev[next] )
                    {
                        ...
                        /*if (rank == 0)
                        {
                           std::cout<<rank <<" Tracker check" <<total_subjects<< " "<<next <<" "<< set_of_distinct_kmers_rev[next]<<" "<<  range_start<< " "<< range_end << "\n";
                        }*/
                    }
                    /*if (rank == 0)
                    {
                           std::cout<<rank <<" Tracker check" <<total_subjects<< " "<<next <<" "<<min_strobe<<" "<< set_of_distinct_kmers_rev[next]<<" "<<  range_start<< " "<< range_end << "\n";
                    }*/
                 }
                 if (set_of_distinct_pos_rev[next] >= range_end)
                 {
                 ...
                 /*if (rank == 0)
                 {
                     std::cout<<rank <<" Tracker check" <<total_subjects<< " "<<next <<" "<< kmer_1_pos<<" "<<  range_start<< " "<< range_end << "\n";
                 }*/
            }
        ...
        //if(rank == 0)
        //{
        //std::cout<< rank<<"RS"<<total_subjects<<"Dist_Kmer_Size "<<set_of_distinct_kmers.size()<<" "<< set_of_distinct_pos.size()<<"\n";
        //}
        /*
        if ( rank == 0)
        {
     ...
     /*   if (rank == 0)
        ...
        {
            if ((set_of_distinct_pos[k] - set_of_distinct_pos[j]) <= 1000)
            {
                for(int g = 0; g < node_threashold; g++)
                {
        ...
        Tl.clear();
        Tl.shrink_to_fit();
        /*if(set_of_distinct_kmers.size() > 0)
        {
            std::cout<<"Compression "<<rank<<" "<<contig_len<<" "<<total_subjects+s_index - 1<<" "<<set_of_distinct_kmers.size()<<" "<<final_sync_counter<<"\n";
